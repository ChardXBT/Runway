...
LeeWay is a local editorial conveyor for Qlob YouTube Community posts. It uses the captured Qlob
...
3. Choose `Approve & schedule`, `Reject`, or `Another image`.
4. LeeWay records the decision and immediately presents the next option.
5. Every approval receives the first open 10:00 AM `America/Toronto` slot and enters the persisted
   YouTube publisher queue.

There is no session-size or ten-day horizon cap. LeeWay reserves at most one bot-created slot per
...
LeeWay is single-user and pinned to the Qlob channel. It includes the local archive, style profile,
...
bypass, and more than one LeeWay-generated post per day are out of scope.
